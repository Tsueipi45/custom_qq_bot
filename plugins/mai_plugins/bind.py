...
# 绑定查分器 token
async def bind_divingfish_username(client, message, content, sender=None):
    uname = content.strip()
    _log.info(f"[bind_dftoken] 收到 {sender} 的查分器 username1")

    if not sender:
        return "[ERROR] 未提供 sender ID，无法绑定"

    udata = load_userdata()
    udata.setdefault(sender, {})["nickname"] = uname
    save_userdata(udata)
